...
Description: this script simulates the diffusion in free 3D with different
input parameters, and then compare the MSD by plotting them together.

~~~~~~~~~~~~~ adapted from sim_script.m ~~~~~~~~~~~~~

This script call the simDiff3D function
...
for c = 1: length( Dlist)
    ...
    tf = simDiff3D( nTracks, nFrames, frameT, expT, dt, D, locErr);
...
title( '3D Brownian Diffusion', 'FontSize', 14)
...
%%
figure( gcf)
ylim( [1e-5 20]), xlim( [1e-2 3]) % for 100 frame, 20ms

...
Author: Yu-Huan Wang (Kim Lab at UIUC) - yuhuanw2@illinois.edu
    ...
    Last update date: 9/5/2025

    ------- this script is adapted from subdiffSim_Blur.m -------

Description: This code simulation the diffusion following fractional
Brownian Motion (fBM), this allows subdiffusion simulation with the effect
of both locErr & motion blur (averaging)


it simulate the diffusion of gene loci in a cell, to test whether
signal intensity (locErr) could change alpha value from 0.4 to 0.25

compareing the locErr effect of small vs. large loci marker
...
frameT = 20e-3;    % frame interval: 500 ms
expT = 20e-3;      % exposure time: 100 ms
dt = expT/4;       % simulation step time: 20 ms
...
D = 7e-4;          % um^2/s
locErr = 10e-3;     % 20 nm, unit: um

% D = 2e-3;  alpha = 0.4;  locErr = 25e-3; % 6tetO
...
fprintf( '   locErr^2 = %d nm^2, motion blur = %.2f nm^2\n\n', (locErr*1e3)^2, 2*D*expT^alpha/ (1+alpha)/ (2+alpha)* 1e6)
...
%% Add Blurring to Tracks & MSD Plotting

close all

f1 = figure( 'Position', [1000 400 420 400]);
colorList = get( gca,'colororder');     colorList = repmat( colorList, [2, 1]);

fitR = 1: 12; % fitting region of MSD

theoFlag = 0; % flag for plotting the theoretical MSD
linearFitFlag = 1; % 1: linear fit, 0: non-linear fit

locErrList = [ 20 40 0]* 1e-3;
% locErrList = 20e-3;
...
for c = 1: length( locErrList)
    ...
    for i = 1: nTracks        
        tf(i).locE = locErr* randn( nFrames, dim); % unit: um
    end
    ...
    for i = 1: nTracks
        ...
        trajTmp = trajLoc{i};
        for tau = 1: nFrames-1
            dr = trajTmp( tau+1:end, :) - trajTmp( 1:end-tau, :);
            dr2 = sum( dr.^2, 2);
    ...
    elseif logical( linearFitFlag)
        % plot linear fit
        tFit = time( 1: round( end/3));  MSDFit =  2* dim* DFit* tFit.^ alphaFit;
        ...
        fun = fittype( 'log( 2*a*(x)^b+c)');
        x0 = [ dim* DFit, alphaFit, 0];
        xmin = [ 0, 0, -inf];
        xmax = [ inf, 2, inf];
        fitR2 = 1: 20;

        f = fit( time(fitR2)', log( eataMSD(fitR2))', fun, 'StartPoint', x0, 'Lower', xmin, 'Upper', xmax);
        DFit = f.a/ dim;  alphaFit = f.b;  %locErrFit = sign( f.c)* sqrt( abs( f.c)); % unit: um
...
subtitle( sprintf( 'Input: \\alpha=%g, D=%.1e, \\sigma varies', alpha, D), 'FontSize', 13)
...
txt = text( 0.05, 0.95, simTxt, 'VerticalAlignment', 'top', 'FontSize', 13, 'Units', 'normalized');

limX = [0.1 30]; limY = [1e-3 0.1]; % 20-200ms
